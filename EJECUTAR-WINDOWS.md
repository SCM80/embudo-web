# 🪟 Ejecutar Embudo en Windows (local)

Embudo es una app web en Python (Streamlit). No es un `.exe` clásico, pero con
...
2. Copia dentro el archivo **`Instalar-Embudo.bat`**.
3. **Haz doble clic** en `Instalar-Embudo.bat`.
...
4. Se abrirá Embudo en `http://localhost:8501`.
...
Si ya tienes la carpeta del proyecto, usa **`Embudo.bat`** (está junto a
...
Clic derecho sobre `Instalar-Embudo.bat` → **Enviar a** → **Escritorio (crear
acceso directo)**. Podrás renombrarlo a "Embudo" y cambiarle el icono.
